(() => {
    ...
    function getRetryAfterSec(res) {
        const v = parseInt(res.headers.get('Retry-After') || '0', 10);
        return Number.isFinite(v) && v > 0 ? v : 0;
    }

    // UX: khoá nút fetch + countdown
    let retryTimer = null;
    function lockFetchFor(seconds, label = 'Chờ') {
        if (!btnFetch) return;
        ...
        retryTimer = setInterval(() => {
            ...
            btnFetch.textContent = left > 0 ? `${label} ${left}s` : 'Tải dữ liệu';
            if (apiMsg && left > 0) apiMsg.textContent = `⏳ Vui lòng thử lại sau ${left}s…`;
            if (left <= 0) {
                clearInterval(retryTimer);
                retryTimer = null;
                btnFetch.disabled = false;
                btnFetch.textContent = 'Tải dữ liệu';
            }
        }, 250);
    }
    ...
    async function tryReadJsonOrText(res) {
    ...
    async function fetchData({ silent } = { silent: false }) {
        ...
        if (cached) {
            if (apiMsg) apiMsg.textContent = 'Đang hiển thị dữ liệu từ cache (15s)';
        ...
        try {
            ...
            // ✅ nếu lỗi HTTP: đọc body từ clone để không consume res thật
            if (!res.ok) {
                const retryAfter = getRetryAfterSec(res);

                let errBody = { message: '' };
                try {
                    errBody = await tryReadJsonOrText(res.clone());
                } catch { /* ignore */ }

                const msgRaw = errBody?.message || errBody?.msg || errBody?.error || '';
                let msg = msgRaw || `HTTP ${res.status}`;

                if (res.status === 429 && retryAfter) msg += ` (thử lại sau ${retryAfter}s)`;
                if (apiMsg) apiMsg.textContent = '❌ ' + msg;
                if (res.status === 429 && retryAfter) {
                    msg += ` (thử lại sau ${retryAfter}s)`;
                    lockFetchFor(retryAfter, 'Chờ');
                }

                // ✅ auto refresh thì đừng spam toast
                if (!silent) toast(msg, 'err');

                updateChart([], [], 0);
                updateStats([], [], 0, guid,
                    f || new Date(Date.now() - quickHours * 3600 * 1000),
                    t || new Date()
                );
                return;
            }

            // ✅ OK: parse json đúng 1 lần
            const j = await res.json();
            ...
            if (apiMsg) apiMsg.textContent = '❌ Lỗi: ' + msg;
            if (!silent) toast('Lỗi tải dữ liệu', 'err');
    ...
    function renderPayload(j, meta, opt) {
        ...
        const code = (typeof j?.code === 'number') ? j.code : (Array.isArray(j?.data) ? 0 : -1);
        if (code !== 0) {
            if (apiMsg) apiMsg.textContent = `API code=${code} message=${j.message || j.msg || j.error || 'error'}`;
            updateChart([], [], 0);
